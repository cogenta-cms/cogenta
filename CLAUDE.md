...
| Élément | Valeur |
|---|---|
| Lot en cours | **L4 — Runtime agentique** (`docs/lots/L4-runtime-agentique.md`) — 21 tâches. **Tâches 1-6 faites.** 1-2 : `packages/agents` — fournisseurs LLM (3 vendeurs normalisés), `runAgentLoop` (plafond de pas, détection de répétition, coût par appel, timeout/annulation, retries scopés). 3 : `assembleContext` (`[CONSTITUTION]→[SITE]→[AGENT]→[TASK]` fixe et balisé, `CONSTITUTION_TEXT` non paramétrable, `DataItem` échappé). 4 : `defineTool`/`createToolRegistry`/`buildManifest` — `buildManifest` est le point réel d'application de la hiérarchie d'autorité (ne construit un `ExecutableTool` que pour les noms accordés). 5 : `src/tools/core/` — `content.*` (interface `ContentServiceLike` locale et étroite, évite de tirer GraphQL/WebAuthn dans `@cogenta/agents`), `media.*` (contre `MediaStore` réel), `site.config_read`, `http.fetch` (domaines autorisés fixés à la construction, GET/HEAD) — la plupart `reversible: false` (approbation humaine forcée, R6) plutôt qu'un `revert` déductible à tort du reçu seul. 6 : `src/audit/with-audit.ts` — `withAudit`/`withAuditForManifest` enveloppent un `ExecutableTool` pour écrire une entrée (acteur, agent, outil, entrée, sortie, durée, succès/échec) via `AuditLogLike` (interface locale, une vraie `@cogenta/auth` `AuditLog` la satisfait déjà — construite générique exactement pour ce second rédacteur) ; un échec d'écriture d'audit ne fait jamais échouer l'action auditée (même règle qu'ADR-0018). Douze codes d'erreur `PROVIDER_*`/`TOOL_*`/`HTTP_FETCH_*` ajoutés à `@cogenta/core` (tâches 1-5, aucun nouveau pour la 6). Reste : tâches 7-21 (traces, budgets, autonomie, sous-agents, skills, mémoire, RAG, MCP, bac à sable, etc.) |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu), L2 (admin, 16/16 tâches). 1982 tests unitaires, tous verts (intégration Postgres/MySQL/MariaDB du `MediaStore` écrite mais non exécutée cette session — Docker Desktop indisponible dans l'environnement ; les adaptateurs de fournisseurs LLM n'ont pas non plus de test d'intégration exécuté — nécessite une clé API réelle, `vitest.integration.config.ts` prêt, skip loud si absente). |

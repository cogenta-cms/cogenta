...
function page(blocks: readonly VocabularyBlock[]): string {
  return serialize(renderPage({ title: 'Working together, close to home', blocks }, ctx, entries))
}
...
describe('renderPage', () => {
  ...
  it('renders the page title as the h1 when the page has no hero', () => {
    ...
    expect(withoutHero).toContain('<h1 class="cg-page__title">Working together, close to home</h1>')
  })
  ...
  it('passes the fetched entries through to the collectionList block by key', () => {
    expect(FULL_PAGE).toContain('Community clean-up day')
  })

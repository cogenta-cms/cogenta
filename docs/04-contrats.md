...
`schema@2.1 → 2.2` (fiche 42 tâche 2) : ajout du décorateur `strikethrough` à
`RICH_TEXT_DECORATORS` et d'un nouveau type de nœud de bloc `hr` (trait horizontal,
`z.strictObject({ _key, _type: 'hr' })`) au format de texte riche. **Additif, sans

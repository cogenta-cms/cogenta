...
| Élément | Valeur |
|---|---|
| Lot en cours | **L4 — Runtime agentique** (`docs/lots/L4-runtime-agentique.md`) — 21 tâches. **Tâches 1-7 faites.** 1 fournisseurs LLM (3 vendeurs normalisés) · 2 `runAgentLoop` (plafond de pas, détection de répétition, coût/appel, timeout/annulation) · 3 `assembleContext` (`[CONSTITUTION]→[SITE]→[AGENT]→[TASK]` fixe et balisé) · 4 `defineTool`/`createToolRegistry`/`buildManifest` (point réel d'application de la hiérarchie d'autorité) · 5 `src/tools/core/` (`content.*`, `media.*`, `site.config_read`, `http.fetch` — la plupart `reversible: false`, R6) · 6 `src/audit/with-audit.ts` (une entrée par appel d'outil via `AuditLogLike`, échec d'écriture jamais fatal à l'action, ADR-0018) · 7 `src/trace/` (`Trace` = `RunResult` horodaté ; `TraceStore` avec deux implémentations à contrat partagé — mémoire et fichier JSON, R1 — `list`/`get`/`prune` ; `captureTrace` enveloppe un run, échantillonnage par `sampleRate`, jamais l'échec du run lui-même). Douze codes d'erreur `PROVIDER_*`/`TOOL_*`/`HTTP_FETCH_*` ajoutés à `@cogenta/core` (tâches 1-5, aucun nouveau pour 6-7). Reste : tâches 8-21 (budgets, autonomie, sous-agents, skills, mémoire, RAG, MCP, bac à sable, etc.) |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu), L2 (admin, 16/16 tâches). 1999 tests unitaires, tous verts (intégration Postgres/MySQL/MariaDB du `MediaStore` écrite mais non exécutée cette session — Docker Desktop indisponible dans l'environnement ; les adaptateurs de fournisseurs LLM n'ont pas non plus de test d'intégration exécuté — nécessite une clé API réelle, `vitest.integration.config.ts` prêt, skip loud si absente). |

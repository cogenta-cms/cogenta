...
describe('the dark palette is designed, not inverted', () => {
  ...
  it('keeps canvas and surface flush in both schemes — depth is a shadow here, not a lightness step', () => {
  ...
  it('keeps the canvas warm rather than a flat, colourless charcoal', () => {
    // A forest-tinted dark canvas, not "generic dark mode grey" — checked as
    // a real chroma floor, something a mechanical `#111`-style inversion
    // would fail outright (chroma 0).
    expect(toOklch(color('--cg-canvas', 'dark')).c).toBeGreaterThan(0.005)
  })

...
/**
 * The design system (`src/styles/tokens.css`) is the layer every block inherits
 * from, and it is written entirely as functions of contract D's closed token
 * set. Two things can go wrong there and neither shows up in a snapshot:
 *
 * 1. **A misspelt skin variable.** `--cogenta-color-accentFg` looks right and
 *    resolves to nothing — `renderSkin` kebab-cases the token names, so the
 *    real property is `--cogenta-color-accent-fg`. A theme referencing the
 *    camelCase form renders with that colour simply missing, silently.
 * 2. **A derived colour that fails contrast.** Every value here is a `color-mix`
 *    or a relative `oklch()`, so no one can read the file and know the ratio.
 *
 ...
 * *theme* invents on top of it — a muted ink on a sunken panel, a tinted badge,
 * a lifted dark accent — none of which the skin validator ever sees.
 */
...
/**
 * The lot asks for "un mode sombre réel, pas une inversion mécanique". That is
 * a claim about *values*, so it is checked as one: each assertion below is
 * something a mechanical swap of `bg` and `fg` would fail.
 */
describe('the dark palette is designed, not inverted', () => {
  ...
  it('lifts the accent instead of carrying the light one across', () => {

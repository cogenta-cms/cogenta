...
describe('the site settings screen — Navigation tab (fiche 22 tâche 8, part 3)', () => {
  it('lists every sidebar section, all shown by default', async () => {
    ...
    const commerce = (await screen.findByLabelText('Commerce')) as HTMLInputElement
  ...
  it('hides a section for everyone, site-wide, once turned off here', async () => {
    ...
    await waitFor(() => {
      expect((screen.getByLabelText('Commerce') as HTMLInputElement).checked).toBe(false)
    })
  ...
  it('moves a section up, persisting the new order', async () => {
    ...
    fireEvent.click(screen.getByRole('tab', { name: 'Navigation' }))
    // "Commerce" is both a sidebar section heading and, structurally, a
    ...
    const commerceCheckbox = (await screen.findByLabelText('Commerce')) as HTMLInputElement
    ...
    if (upButton === undefined) throw new Error('expected an "up" button in the Commerce row')
    ...
    await waitFor(() => {
      ...
      // Commerce (shipped 3rd) swaps with Appearance (shipped 2nd) — proven
      ...
      // already satisfy.
      expect(labels.indexOf('Commerce')).toBeLessThan(labels.indexOf('Apparence'))
    })
  })
})

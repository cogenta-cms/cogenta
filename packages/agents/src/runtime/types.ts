...
export interface RunAgentLoopInput {
  ...
  /** Per model call, including retries. Absent falls back to `client.maxCorrectionAttempts` — always a concrete number, see `ProviderClient`'s own doc comment. */
  readonly maxAttempts?: number
  /** Per model call attempt. Absent falls back to `client.requestTimeoutMs` — always a concrete number, see `ProviderClient`'s own doc comment. */
  ...
  /**
   * Fiche feedback — a real user report that a chat turn or an agent run
   * gave no sign of life while it worked. Called at each meaningful step
   * (a model call starting, a tool being dispatched, a retry after an
   * error) — a pure side effect, never something the loop's own control
   * flow reads back, so a caller with nothing to show progress to (most
   * tests, `runSubagent`'s inner call) can simply omit it.
   */

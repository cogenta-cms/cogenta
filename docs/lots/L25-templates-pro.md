...
| Passe D5 — zéro dégradé sur l'existant | en cours | A0e fait (`5c7fbd8`, 5 thèmes L23). A0d fait (`27cfae7`) : `demo-art` réécrit en compositions plates (blocs de couleur, grille & nœud, bandes, anneaux, marque éditoriale, arche & soleil, empilement, damier, blobs duotone), zéro `gradient`/`glow` dans les presets (test `flat-design.test.ts`) ; **correction de la session principale** `aeb0e3d` : les heros étaient aux deux tiers vides (« zone de texte calme » à gauche, alors que les thèmes cadrent le média du hero à côté du titre) → composition pleine toile. A0e-bis en cours sur les 5 nouveaux thèmes |
| Phase 1 vague 2 — entreprise, magazine, portfolio, ecommerce (passes pro) | à faire | avec la règle D5 dans le brief |

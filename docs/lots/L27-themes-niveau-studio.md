...
| CI instable (tests admin trop pressés, `mkdir` non attendu dans `@cogenta/agents`) | fait | `ca9e31a`, `050485d`, `9d4ac13` ; balayage des tests admin reproduit sous délai simulé |
...
| `saas` | fait | Geist + Geist Mono, blanc/gris structuré, bleu signal unique ; hero aligné à gauche avec une vraie capture d'application (file d'approbations) rendue en PIL, six captures produit, six wordmarks clients, fonctionnalités 3×2 sans tuiles, étapes numérotées, tableau comparatif de tarifs à colonnes partagées (écart mesuré ramené de 36 px à 0), CTA honnêtes vers `/demo` ; 322 tests. Revu en captures, une reprise (alignement du tableau de tarifs), agent coupé par une limite de session puis relancé sur le même worktree |
| `association` | fait | Bricolage Grotesque + Source Sans 3, papier chaud, vert profond, jaune signal réservé au don ; en-tête photo avec titre sur papier (jamais de texte sur photo), chiffres d'impact avec phrase de contexte, programmes en rangées image/texte, agenda à bloc de date typographique et page d'événement lue depuis `theme@1.5`, répartition des dépenses en barres, bandeau de dons à montants concrets, dons et bénévolat honnêtes (virement, e-mail, soirée d'orientation) ; nom = nom du site ; photos retouchées pour retirer lettrages ; 345 tests. Revu en captures, une reprise (heures au format du texte, événements sans saison) |
| `docs` (documentation) | fait | IBM Plex Sans + Plex Mono, bleu pétrole unique ; accueil à recherche large, « Start here » sans tuiles, documentation par sections ; page doc à trois colonnes (navigation latérale collante, article 56ch, « On this page »), fil d'Ariane, blocs de code étiquetés lus depuis le texte riche existant, notes, tableaux de référence, précédent/suivant, date de mise à jour ; commentaires retirés des pages de doc ; produit fictif = serveur de livraison de webhooks nommé d'après le site, 13 pages cohérentes avec code shell/YAML/JSON/TS/Python ; 296 tests. Revu en captures, fusionné sans reprise |
| `canonical` | fait | Instrument Sans (+ Instrument Serif pour la seule ligne d'affichage et les citations), neutres purs, bleu profond réservé aux liens ; grille 12 col mesurée, rythme unique, filets à la place des cartes, mode sombre dérivé de l'encre du skin, lisible sous un skin étranger (contraste AA testé sous deux skins tiers) ; attributs du constructeur de pages conservés, fidélité à l'octet vérifiée ; 310 tests. Revu en captures, fusionné sans reprise |
| Galerie : chaque thème prévisualisé avec son propre skin, page d'aperçu crédible | fait | `22b9cc7`, `a5ad5d1` — la route promettait le skin du thème mais rendait celui du site |
| Identité de thème à la sélection, vérification finale | fait | 2026-09-14 — la galerie prévisualise chaque thème avec son propre skin, la sélection demande s'il faut l'appliquer ; suite en L28 (données d'exemple) |

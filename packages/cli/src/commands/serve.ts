...
/**
 * `POST /api/forms/{name}/submit` must work with a plain HTML `<form>`
 * (fiche 16 task 3: "fonctionne sans JavaScript") — which posts
 * `application/x-www-form-urlencoded`, not JSON. Every other route on this
 * server only ever receives JSON from the admin SPA, so this parsing lives
 * here rather than in the shared `readBody`.
 */
async function readFormAwareBody(req: IncomingMessage): Promise<unknown> {
  const contentType = req.headers['content-type'] ?? ''
  if (!contentType.includes('application/x-www-form-urlencoded')) return readBody(req)
  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/forms')) {
        ...
        const body = isHtmlSubmit
          ? await readFormAwareBody(req)

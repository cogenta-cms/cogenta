...
| Élément | Valeur |
|---|---|
| Lot en cours | **L4 — Runtime agentique** (`docs/lots/L4-runtime-agentique.md`) — 21 tâches. **Tâche 1 faite** : `packages/agents` créé, abstraction fournisseurs LLM (`ProviderClient`) normalisant le tool-calling sur trois vendeurs aux formats de fil différents (Anthropic `tool_use`, OpenAI `tool_calls`, Google Gemini `functionCall`/pas d'id d'appel — id synthétisé depuis le nom de fonction) ; chaque adaptateur sépare mapping pur (`build*Request`/`parse*Response`, testé par fixtures, sans réseau) et enveloppe `fetch` fine (testée avec un `fetchImpl` injecté) ; `createProviderRegistry` ne construit que les clients dont la config est fournie — un site sans fournisseur a un registre vide, conforme à R2. Quatre codes d'erreur ajoutés à `@cogenta/core` (`PROVIDER_*`). Reste : tâches 2-21 (boucle d'exécution, permissions, sous-agents, mémoire, RAG, MCP, bac à sable, etc.) |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu), L2 (admin, 16/16 tâches). 1908 tests unitaires, tous verts (intégration Postgres/MySQL/MariaDB du `MediaStore` écrite mais non exécutée cette session — Docker Desktop indisponible dans l'environnement ; les adaptateurs de fournisseurs LLM n'ont pas non plus de test d'intégration exécuté — nécessite une clé API réelle, `vitest.integration.config.ts` prêt, skip loud si absente). |

...
- 2026-09-02 ~07:30 — **Vague A entièrement terminée, plus aucun agent actif** (consigne utilisateur : ne plus lancer d'agents). Branches prêtes, **non fusionnées**, en attente de décision :
  - A1 commerce `worktree-agent-aac6f1e0f6cb3a596` (5 commits, commerce 307/307) — facturation/relance/renouvellement planifiés, changement de formule, recherche de commandes, tests RGPD client, abandon auto des paniers.
  - A2 RGPD/audit/auth `worktree-agent-a028676f7a734650b` (3 commits, api 1218 + core 501 verts) — export RGPD, purge d'audit planifiée, `Retry-After`, portée de clé lisible. Signale un échec préexistant `serve-users.test.ts` (e-mail anonymisé rejournalisé).
  - A4 rendu public `worktree-agent-a15d31b38be4e1d78` (4 commits, thèmes 1234 verts, `theme@1.3`) — identité du site dans le chrome, archives de termes, RSS/Atom, barre d'admin (jamais rendue auparavant), polices. Bug transverse non corrigé : stores créés sans `defaultLocale` dans `serve.ts`.
  - A5 médiathèque `worktree-agent-aebd910e16b0a2b10` (6 commits) — upload multipart réel, cache-bust `&v=`, dossier dans le sélecteur, usage avant suppression groupée, filtre date. Non fait : tests `replace`/EXIF, aperçu vidéo/PDF. Trou trouvé : `POST /api/media` ignore `folderId`.
  - A6 finitions admin `worktree-agent-a3160b9d5d60e42e5` (10 commits, 67 tests admin verts) — les 9 tâches faites.
  Fusionné : A3 uniquement. Vagues B et C non lancées.

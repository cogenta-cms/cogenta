...
| Passe D5 — zéro dégradé sur l'existant | en cours | A0e fait et fusionné (`5c7fbd8`) : 5 thèmes L23 à zéro `gradient(` + suppression des `backdrop-filter: blur` (verre dépoli, même famille « lueur IA »), test de garde dans `isolation.test.ts` de chacun ; A0e-bis lancé 2026-09-05 ~13:20 sur les 5 nouveaux thèmes (saas 9, blog 2, restaurant 1, association 1, docs 0 + blurs) ; A0d (visuels plats) toujours en cours |

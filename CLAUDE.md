...
| Élément | Valeur |
|---|---|
| Lot en cours | **L4 — Runtime agentique** (`docs/lots/L4-runtime-agentique.md`) — 21 tâches. **Tâches 1-8 faites.** 1 fournisseurs LLM (3 vendeurs normalisés) · 2 `runAgentLoop` (plafond de pas, détection de répétition, coût/appel, timeout/annulation) · 3 `assembleContext` (`[CONSTITUTION]→[SITE]→[AGENT]→[TASK]` fixe et balisé) · 4 `defineTool`/`createToolRegistry`/`buildManifest` (point réel d'application de la hiérarchie d'autorité) · 5 `src/tools/core/` (`content.*`, `media.*`, `site.config_read`, `http.fetch`) · 6 `src/audit/with-audit.ts` (une entrée par appel d'outil, échec d'écriture jamais fatal, ADR-0018) · 7 `src/trace/` (`Trace`/`TraceStore`, deux implémentations à contrat partagé, `captureTrace`) · 8 `src/budget/` — `createBudgetTracker` (limites `tokensPerDay`/`eurPerMonth`/`callsPerHour` de Contrat C, fenêtres calendaires jour/mois/heure, `costOf` injectable puisque le tarif est une donnée pas du code) et `createKillSwitch` (arrêt humain explicite, indépendant du budget) ; `runAgentLoop` vérifie budget/kill-switch/`maxRunDurationMs` avant **chaque** appel modèle et s'arrête proprement (`budget_exceeded`/`killed`/`max_duration`) plutôt que de dégrader en silence — `maxRunDurationMs` reste un paramètre du runtime, pas du `budget` de Contrat C (qui ne le liste pas), pour ne pas étendre une forme figée. Douze codes d'erreur `PROVIDER_*`/`TOOL_*`/`HTTP_FETCH_*` ajoutés à `@cogenta/core` (tâches 1-5, aucun nouveau pour 6-8). Reste : tâches 9-21 (autonomie, sous-agents, skills, mémoire, RAG, MCP, bac à sable, etc.) |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu), L2 (admin, 16/16 tâches). 2013 tests unitaires, tous verts (intégration Postgres/MySQL/MariaDB du `MediaStore` écrite mais non exécutée cette session — Docker Desktop indisponible dans l'environnement ; les adaptateurs de fournisseurs LLM n'ont pas non plus de test d'intégration exécuté — nécessite une clé API réelle, `vitest.integration.config.ts` prêt, skip loud si absente). |

...
export function renderLogoStrip(block: LogoStripBlock, ctx: RenderContext): HtmlElement {
  return section(
    ...
    block.caption === undefined
      ? null
      : h('p', { class: 'cr-strip__caption', 'data-field': 'caption' }, block.caption),
    h(
      'ul',
      { class: 'cr-strip__items' },
      block.logos.map((logo) =>
        h(
          'li',
          { class: 'cr-strip__item' },
          h(
            'span',
            { class: 'cr-marks__plate' },
            image(ctx, logo.media, { className: 'cr-marks__image', sizes: '10rem' }),
          ),
        ),
      ),
    ),

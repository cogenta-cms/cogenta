...
| Élément | Valeur |
|---|---|
| Lot en cours | **L2 — Admin** (`docs/lots/L2-admin.md`) — les 16 tâches listées sont toutes traitées : …, journal d'audit, tableau de bord (`GET /api/health` admin-only + widgets santé/activité/programmé réels, CVE/Core Web Vitals/sauvegardes vides et explicites faute de source), et **tâche 16 (passe accessibilité WCAG 2.2 AA)** : `axe-core` ajouté en devDependency, `packages/admin/test/accessibility.test.tsx` fait tourner axe (jeu de règles wcag2a/aa, wcag21a/aa, wcag22aa) sur les 7 vues principales dans leur état le plus chargé, zéro violation sérieuse — l'infrastructure statique (skip-link, focus visible, `lang="fr"`, `prefers-reduced-motion`, lint Biome `a11y/*`) était déjà en place depuis les tâches précédentes, task 16 n'ajoutait que la vérification automatisée manquante. Le premier des deux écarts documentés est comblé : **UI d'inscription de passkey** (`/settings`, `registerPasskey()` dans le client admin) — le serveur avait déjà `beginWebAuthnRegistration`/`completeWebAuthnRegistration` et les routes `/api/auth/webauthn/register/{begin,complete}` (construits à l'avance, jamais branchés), il ne manquait que la page. **Traduction de l'interface admin** (reste de la tâche 13) en cours, cadrée par ADR-0019 : `react-i18next`, catalogues `fr.json`/`en.json` sous `packages/admin/src/i18n/locales/`, langue par défaut = préférence enregistrée puis `navigator.language` si `fr`/`en`, sinon français (aucune surprise pour un site déjà déployé) ; sélecteur dans `/settings`. Migrés : nav, coquille, connexion, paramètres, liste des collections, tableau de bord, journal d'audit, liste d'une collection, médiathèque (page + upload + détail + point focal), formulaire d'entrée, sélecteur de traduction, historique de versions. Restent : composants de champ (14 fichiers), éditeur de blocs, éditeur de texte riche — migration progressive assumée par l'ADR elle-même, pas de big bang |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu). 1878 tests unitaires, tous verts (intégration Postgres/MySQL/MariaDB du `MediaStore` écrite mais non exécutée cette session — Docker Desktop indisponible dans l'environnement). |

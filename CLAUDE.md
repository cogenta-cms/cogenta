...
| Élément | Valeur |
|---|---|
| Lot en cours | **L4 — Runtime agentique** (`docs/lots/L4-runtime-agentique.md`) — 21 tâches. **Tâches 1-2 faites.** Tâche 1 : `packages/agents`, abstraction fournisseurs LLM (`ProviderClient`) normalisant le tool-calling sur trois vendeurs aux formats de fil différents (Anthropic `tool_use`, OpenAI `tool_calls`, Google Gemini `functionCall`/pas d'id d'appel — id synthétisé depuis le nom de fonction) ; `createProviderRegistry` ne construit que les clients configurés (registre vide sans fournisseur, conforme à R2). Tâche 2 : `runAgentLoop` (`src/runtime/loop.ts`) — appel modèle → dispatch outil → reprise, avec les trois garde-fous que le lot nomme explicitement dans ses pièges connus : plafond de pas (`maxSteps`), détection de répétition (`RepetitionGuard`, appel identique nom+entrée compté avant exécution, jamais après), coût mesuré par appel dès la première ligne (`usage` agrégé dans `RunResult`) ; retries seulement sur erreurs plausiblement transitoires (`PROVIDER_REQUEST_FAILED`/`RATE_LIMITED`/`TIMEOUT`, backoff injectable) ; timeout par appel distinct de l'annulation externe (`AbortSignal`) ; un outil absent ou qui lève une exception devient un résultat d'échec renvoyé au modèle, jamais un crash du run. Six codes d'erreur ajoutés à `@cogenta/core` (`PROVIDER_*`). Reste : tâches 3-21 (contexte/hiérarchie d'identité, registre d'outils et permissions, outils du noyau, audit, traces, budgets, autonomie, sous-agents, skills, mémoire, RAG, MCP, bac à sable, etc.) |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu), L2 (admin, 16/16 tâches). 1929 tests unitaires, tous verts (intégration Postgres/MySQL/MariaDB du `MediaStore` écrite mais non exécutée cette session — Docker Desktop indisponible dans l'environnement ; les adaptateurs de fournisseurs LLM n'ont pas non plus de test d'intégration exécuté — nécessite une clé API réelle, `vitest.integration.config.ts` prêt, skip loud si absente). |

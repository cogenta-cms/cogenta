...
| Élément | Valeur |
|---|---|
| Lot en cours | **L2 — Admin** (`docs/lots/L2-admin.md`) — tâches 1-8 faites (coquille, auth complète, schéma + permissions, composants de champ, liste de collection, formulaire d'édition/création généré depuis schema.json, éditeur de texte riche structuré sur Slate) ; reste pour la tâche 3 : UI d'inscription de passkey (page réglages pas encore construite) ; tâche 9 (éditeur de blocs) à faire ensuite |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu). 1805 tests unitaires, tous verts. |

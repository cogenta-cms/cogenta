...
| Élément | Valeur |
|---|---|
| Lot en cours | **L4 — Runtime agentique** (`docs/lots/L4-runtime-agentique.md`) — 21 tâches. **Tâches 1-4 faites.** Tâches 1-3 : `packages/agents` — abstraction fournisseurs LLM (3 vendeurs normalisés), `runAgentLoop` (plafond de pas, détection de répétition, coût par appel, retries scopés, timeout distinct de l'annulation), `assembleContext` (`[CONSTITUTION] → [SITE] → [AGENT] → [TASK]` fixe et balisé, `CONSTITUTION_TEXT` non paramétrable, `DataItem` échappé et jamais concaténé au `system`). Tâche 4 : `src/tools/` — `defineTool` (Contrat C `tools@1.0`, valide structurellement `sideEffects+reversible ⇒ revert` et `permissions` non vide, gèle l'objet retourné) ; `createToolRegistry` (index par nom, doublon refusé) ; `buildManifest(registry, allowedNames, ctx)` **est** le point d'application réel de la hiérarchie d'autorité : ne construit un `ExecutableTool` que pour les noms explicitement accordés — un outil hors manifeste n'existe simplement pas pour le modèle, peu importe le prompt — validation Zod de l'entrée/sortie à chaque appel (`z.toJSONSchema` pour le `ProviderToolSpec` exposé au modèle), nom manquant dans le registre refusé au chargement, pas à l'exécution. Onze codes d'erreur `PROVIDER_*`/`TOOL_*` ajoutés à `@cogenta/core` (tâches 1-4). Reste : tâches 5-21 (outils du noyau, audit, traces, budgets, autonomie, sous-agents, skills, mémoire, RAG, MCP, bac à sable, etc.) |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu), L2 (admin, 16/16 tâches). 1954 tests unitaires, tous verts (intégration Postgres/MySQL/MariaDB du `MediaStore` écrite mais non exécutée cette session — Docker Desktop indisponible dans l'environnement ; les adaptateurs de fournisseurs LLM n'ont pas non plus de test d'intégration exécuté — nécessite une clé API réelle, `vitest.integration.config.ts` prêt, skip loud si absente). |

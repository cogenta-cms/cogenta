...
| Élément | Valeur |
|---|---|
| Lot en cours | **L4 — Runtime agentique** (`docs/lots/L4-runtime-agentique.md`) — 21 tâches, le lot le plus dense. Contrat C figé (ADR-0020), démarrage de la tâche 1 (abstraction fournisseurs LLM, normalisation du tool-calling). Aucun code de départ n'existait (`packages/agents/`, `packages/mcp/` à créer) sauf le journal d'audit (`packages/auth/src/audit.ts`) déjà construit pour servir de second rédacteur à L4 |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu), **L2 (admin, 16/16 tâches)**. 1878 tests unitaires, tous verts (intégration Postgres/MySQL/MariaDB du `MediaStore` écrite mais non exécutée cette session — Docker Desktop indisponible dans l'environnement). |

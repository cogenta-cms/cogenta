...
function readEntryData(buffer: Buffer, entry: CentralEntry): Buffer {
  ...
  if (entry.uncompressedSize > MAX_INFLATED_BYTES) {
    throw new CogentaError({
      ...
      message: `"${entry.name}" expands to ${entry.uncompressedSize} bytes, over the ${MAX_INFLATED_BYTES}-byte limit.`,
...
export interface ZipArchive {
  ...
  /** `undefined` when the archive has no such entry — a missing part is a caller's decision, not an error here. */
...
export function openZip(buffer: Buffer): ZipArchive {
  ...
  return {
    ...
    read(name) {
      ...
      return entry === undefined ? undefined : readEntryData(buffer, entry)
    },
  }
}

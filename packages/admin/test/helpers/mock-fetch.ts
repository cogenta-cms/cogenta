...
export function installMockFetch(
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      ...
      // A `FormData` body (fiche 46's `replaceMedia`, the one media write
      // that has to be multipart on the wire) is not JSON — parsed as `{}`
      // here, and read back out of `init.body` directly by the one handler
      // that needs it, the same way the real router treats it as a second,
      ...
      if (mediaFoldersMatch !== null) {
        ...
        if (folderId !== undefined && moveAction === 'move' && method === 'POST') {
          ...
          if (target !== undefined && target.path.startsWith(found.path)) {
            return json(400, {
              error: {
                code: 'MEDIA_FOLDER_CYCLE',

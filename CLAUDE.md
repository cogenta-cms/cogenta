...
| Élément | Valeur |
|---|---|
| Lot en cours | **L4 — Runtime agentique** (`docs/lots/L4-runtime-agentique.md`) — 21 tâches. **Tâches 1-3 faites.** Tâches 1-2 : `packages/agents` — abstraction fournisseurs LLM (`ProviderClient`, 3 vendeurs normalisés) et `runAgentLoop` (appel → outils → reprise, plafond de pas, détection de répétition, coût mesuré par appel, retries scoping aux erreurs transitoires, timeout distinct de l'annulation). Tâche 3 : `src/identity/` — `assembleContext` construit `[CONSTITUTION] → [SITE] → [AGENT] → [TASK]` en ordre fixe et balisé (`system` du `ChatRequest`) ; `CONSTITUTION_TEXT` est un texte fixe interne au module, **jamais un paramètre** (aucun niveau inférieur ne peut l'atteindre, donc aucun ne peut le surcharger) ; le contenu externe (`DataItem`) devient des messages `dataMessages` séparés, jamais concaténés au `system`, avec échappement de `<`/`>`/`"` pour empêcher une injection de fermer prématurément son propre bloc ou d'en ouvrir un nouveau (`<task>`, `<agent>`…) — conforme à R8. Six codes d'erreur `PROVIDER_*` ajoutés à `@cogenta/core` (tâches 1-2). Reste : tâches 4-21 (registre d'outils et permissions — c'est là que la hiérarchie d'autorité est *réellement* appliquée, pas dans le prompt —, outils du noyau, audit, traces, budgets, autonomie, sous-agents, skills, mémoire, RAG, MCP, bac à sable, etc.) |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu), L2 (admin, 16/16 tâches). 1938 tests unitaires, tous verts (intégration Postgres/MySQL/MariaDB du `MediaStore` écrite mais non exécutée cette session — Docker Desktop indisponible dans l'environnement ; les adaptateurs de fournisseurs LLM n'ont pas non plus de test d'intégration exécuté — nécessite une clé API réelle, `vitest.integration.config.ts` prêt, skip loud si absente). |

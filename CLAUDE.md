...
| Élément | Valeur |
|---|---|
| Lot en cours | **L2 — Admin** (`docs/lots/L2-admin.md`) — tâches 1-15 faites (…, journal d'audit, **tableau de bord** : `GET /api/health` réservé au rôle `admin` expose le même rapport driver/tier/latence que `cogenta doctor`, widgets réels pour santé du site + activité récente (réutilise `/api/audit`) + contenus programmés (liste filtrée par `status=scheduled` sur les collections où l'acteur a un accès brouillon), CVE/Core Web Vitals/sauvegardes laissés vides et explicites faute de source de données — même règle déjà appliquée aux blocs liés aux agents) ; reste pour la tâche 3 : UI d'inscription de passkey ; traduction de l'interface admin elle-même toujours non traitée (tâche 13) ; tâche 16 (passe accessibilité WCAG 2.2 AA) à faire ensuite, dernière tâche du lot |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu). 1867 tests unitaires, tous verts (intégration Postgres/MySQL/MariaDB du `MediaStore` écrite mais non exécutée cette session — Docker Desktop indisponible dans l'environnement). |

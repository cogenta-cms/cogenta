...
| Élément | Valeur |
|---|---|
| Lot en cours | **L2 — Admin** (`docs/lots/L2-admin.md`) — tâches 1-13 faites (…, prévisualisation, **i18n** : `GET /{collection}/{id}/translations` liste la famille de traduction (ADR-0014), `site.locales`/`defaultLocale` exposés par `/api/schema` (bug corrigé au passage : `cogenta serve` les ignorait), sélecteur de traduction dans l'admin créant une nouvelle langue pré-remplie depuis la source ; traduction de l'interface admin elle-même **non traitée** — aucune infrastructure de chaînes/dictionnaire n'existe, décision à prendre séparément) ; reste pour la tâche 3 : UI d'inscription de passkey ; tâche 14 (journal d'audit) à faire ensuite |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu). 1854 tests unitaires, tous verts (intégration Postgres/MySQL/MariaDB du `MediaStore` écrite mais non exécutée cette session — Docker Desktop indisponible dans l'environnement). |

...
import { webFontImports } from './web-fonts.js'
...
export function renderSkinCss(tokens: SkinTokens, options: RenderSkinCssOptions = {}): string {
  ...
  return [
    // First, where CSS still honours an `@import`: the web fonts this skin's
    // stacks lead with, so a typeface a skin names is the typeface a reader
    // sees (L29 D1). A stack outside the catalogue adds nothing.
    ...webFontImports(tokens.font),

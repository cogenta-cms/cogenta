...
| Élément | Valeur |
|---|---|
| Lot en cours | **L2 — Admin** (`docs/lots/L2-admin.md`) — tâches 1-14 faites (…, i18n, **journal d'audit** : `@cogenta/auth` avait déjà le stockage à chaînage de hash sans aucun rédacteur ni route — connexions/déconnexions et écritures de contenu/média enregistrées automatiquement à la couche transport de `cogenta serve`, `GET /api/audit` (filtrable) et `/api/audit/verify` réservés au rôle `admin` (ADR-0018), UI admin avec filtres + vérification d'intégrité) ; reste pour la tâche 3 : UI d'inscription de passkey ; traduction de l'interface admin elle-même toujours non traitée (tâche 13) ; tâche 15 (tableau de bord) à faire ensuite |
| Lots terminés | L0 (socle), L1 (contenu), L3 (rendu). 1863 tests unitaires, tous verts (intégration Postgres/MySQL/MariaDB du `MediaStore` écrite mais non exécutée cette session — Docker Desktop indisponible dans l'environnement). |

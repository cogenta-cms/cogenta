 ...
 * heading. A CSS-drawn plus mark (rotated to a cross on `[open]`) replaces
 * the native marker rather than a chevron — the theme's own vocabulary of
 * shapes, kept purely visual.
 */
function renderItem(item: FaqItem, ctx: RenderContext): HtmlElement {
  return h(
    ...
    h(
      ...
      h('summary', { class: 'cg-faq__question' }, h('span', {}, item.question)),
